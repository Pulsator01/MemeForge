...
class ZerePyCLI:
    ...
    def _initialize_commands(self) -> None:
        ...
        # Load multi-agent file command
        self._register_command(
            Command(
                name="load-multi-agents",
                description="Loads multiple agents from a JSON file.",
                tips=["Format: load-multi-agents {file_name}",
                      "The file should contain an 'agents' array with agent definitions"],
                handler=self.load_multi_agents,
                aliases=['load-many', 'load-all']
            )
        )
        
        # Start all agents command
        self._register_command(
            Command(
                name="start-all-agents",
                description="Starts all loaded agents in parallel.",
                tips=["Use 'load-multi-agents' first to load agents",
                      "Press Ctrl+C to stop all agents"],
                handler=self.start_all_agents,
                aliases=['start-all', 'run-all']
            )
        )
        ...
        self._register_command(
            Command(
                ...
                description="Stops all running agents.",
                tips=["This will gracefully stop all running agents"],
                handler=self.stop_all_agents,
                aliases=['stop-all']
            )
        )
        
        # List running agents command
        self._register_command(
            Command(
                name="list-running-agents",
                description="Lists all currently running agents.",
                tips=["Shows which agents are actively running"],
                handler=self.list_running_agents,
                aliases=['running-agents', 'ls-running']
            )
        )
    ...
    def load_multi_agents(self, input_list: List[str]) -> None:
        """Handle load multi-agents command"""
        if len(input_list) < 2:
            logger.info("Please specify a file name.")
            logger.info("Format: load-multi-agents {file_name}")
            return
        ...
        try:
            ...
            loaded_agents = self.multi_agent_manager.load_agents_from_file(input_list[1])
            logger.info(f"\n✅ Successfully loaded {len(loaded_agents)} agents:")
            for agent_name in loaded_agents:
                logger.info(f"  - {agent_name}")
        except Exception as e:
            logger.error(f"Error loading agents: {e}")

    def start_all_agents(self, input_list: List[str]) -> None:
        """Handle start all agents command"""
        if self.multi_agent_manager is None or not self.multi_agent_manager.get_loaded_agents():
            logger.info("No agents loaded. Use 'load-multi-agents' first.")
            return

        try:
            self.multi_agent_manager.start_all_agents()
            logger.info("\n🚀 All agents started!")
    ...
    def stop_all_agents(self, input_list: List[str]) -> None:
        """Handle stop all agents command"""
        if self.multi_agent_manager is None:
            return

        try:
            self.multi_agent_manager.stop_all_agents()
            logger.info("\n🛑 All agents stopped.")
        except Exception as e:
            logger.error(f"Error stopping agents: {e}")

    def list_running_agents(self, input_list: List[str]) -> None:
        """Handle list running agents command"""
        if self.multi_agent_manager is None:
            logger.info("No agents loaded. Use 'load-multi-agents' first.")
            return

        running_agents = self.multi_agent_manager.get_running_agents()
