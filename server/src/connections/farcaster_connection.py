...
class FarcasterConnection(BaseConnection):
    ...
    def register_actions(self) -> None:
        ...
        self.actions = {
            ...
            "get-mentions": Action(
                name="get-mentions",
                parameters=[
                    ActionParameter("fid", True, int, "Farcaster ID to get mentions for"),
                    ActionParameter("limit", False, int, "Number of mentions to fetch, defaults to 25")
                ],
                description="Get recent mentions for a user"
            )
        }
    ...
    def get_mentions(self, fid: int, limit: int = 25) -> List[Dict[str, Any]]:
        """Get recent mentions for a user"""
        logger.debug(f"Getting mentions for FID {fid}")
        # Get recent casts from timeline and filter for mentions
        recent_casts = self._client.get_mention_and_reply_notifications(cursor=None, limit=limit)
        mentions: List[Dict[str, Any]] = []
        
        try:
            for cast in recent_casts:
                logger.info(f">>> cast: {cast}")
                # Cast objects should have a text attribute or method to get text
                cast_text = getattr(cast, 'text', None)
                if cast_text and f"@{fid}" in cast_text:
                    # Convert cast to dictionary format
                    cast_dict = {
                        'hash': getattr(cast, 'hash', ''),
                        'text': cast_text,
                        'author_fid': getattr(cast, 'author_fid', 0)
                    }
                    mentions.append(cast_dict)
                
                if len(mentions) >= limit:
                    break
        except Exception as e:
            logger.error(f"Error processing casts: {e}")
